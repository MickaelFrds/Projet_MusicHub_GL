...
public class MusicHub {
    ...
    public void deleteAlbum(String albumTilte) throws NoAlbumFoundException {
        Album theAlbum = null;
        for (Album al : albums) {
            if (al.getTitle().toLowerCase().equals(albumTilte.toLowerCase())) {
                theAlbum = al;
                break;
            }
        }
    ...
    public String getTitleSongs() {
        ...
        for (AudioElement el : elements) {
            if (el instanceof Song) {
                titleList.append(el.getTitle()).append("\n");
            }
        }
    ...
    public void addElementToAlbum(String elementTitle, String albumTitle) throws NoAlbumFoundException, NoElementFoundException {
        UUID elementUuid = elements.stream()
                .filter(e -> elementTitle.equalsIgnoreCase(e.getTitle()))
                .map(AudioElement::getUUID)
                .findFirst()
                .orElseThrow(() -> new NoElementFoundException("Element " + elementTitle + " not found!"));

        Album album = this.albums.stream()
                .filter(a -> albumTitle.equalsIgnoreCase(a.getTitle()))
                .findFirst()
                .orElseThrow(() -> new NoAlbumFoundException("Album " + albumTitle + " not found!"));

        album.addSong(elementUuid);
    }
